...
if [ $machine == "Linux" ]; then
    echo "set config for Linux..."
fi

ETC=$HOME/.local/config/etc
BIN=$HOME/.local/config/bin
CONFIG=$HOME/.local/config/sample
mkdir -p $ETC
mkdir -p $BIN
mkdir -p $CONFIG
...
if [ -d dotfiles ]; then
    ...
    git clone https://gitee.com/Kiraraako/dotfiles
    cd dotfiles
fi
cp -rf etc/* $ETC/
cp -rf bin/* $BIN/
...
echo "source $BIN/init.sh" >> ~/.bashrc
source $HOME/.bashrc
